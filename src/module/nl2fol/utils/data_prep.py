...
from .misc import all_exists, any_exists
...
class RefineFOLDataPreparer(DataPreparer):

    template_name = 'refine_fol_prompt_template'
    ...
    def make_prompts(
            ...
            nl_key: Optional[str] = None,
            fol_key: Optional[str] = None,
            predicate_fol_key: Optional[str] = None,
            **kwargs
    ):
        """
            generate prompt for translation task; supports generating either input_prompt or output_prompt or both
            ...
            Returns:
                ...
                output_prompt: output prompt or '' if fol_key not given
        """
        assert all_exists(nl_key, predicate_fol_key) or  all_exists(fol_key), 'at least give me one key'
        input_prompt, output_prompt = self.prompter.generate_prompt(
            
            template_name=self.template_name,
            input_kwargs={
                'nl': data_point[nl_key],
                'predicate_fol': data_point[predicate_fol_key]
            } if all_exists(nl_key, predicate_fol_key) else None,
            output_kwargs= None
        )
